...
void draw_marquee_list_item(AppContext& context, int marquee_index, int x, int y, const String& text, bool selected) {
    context.display.setTextWrap(false); // Prevent text wrapping
    ...
    int marquee_x = x;
    if (w > 128 - x) {
        if (!context.is_marquee_active[marquee_index]) {
            context.is_marquee_active[marquee_index] = true;
            context.marquee_start_time[marquee_index] = millis();
            context.marquee_text[marquee_index] = text;
        }

        unsigned long elapsed = millis() - context.marquee_start_time[marquee_index];
        ...
        context.is_marquee_active[marquee_index] = false;
    }
...
void draw_list_ui(AppContext& context, const String& title, const std::vector<String>& items, int selected_item, const String& bottom_item_text) {
    ...
    if (items.empty() && bottom_item_text.isEmpty()) {
        ...
        for (int i = 0; i < list_size + (bottom_item_text.isEmpty() ? 0 : 1); i++) {
            ...
            if (y_pos > 12 && y_pos < 64 && marquee_line_index < AppContext::MAX_MARQUEE_LINES) {
                String item_text = (i == list_size) ? bottom_item_text : items[i];

                if (!item_text.isEmpty()) {
                   draw_marquee_list_item(context, marquee_line_index, 12, y_pos, item_text, i == selected_item);
                   marquee_line_index++;
                }
            }
        }
...
void draw_player_ui(AppContext& context) {
    ...
    if (!context.current_playlist_files.empty()) {
        ...
        context.display.drawRect(0, 24, 128, 5, SSD1306_WHITE);
        ...
        for (int i = 0; i < list_size + 1; i++) {
            ...
            if (y_pos > 30 && y_pos < 64 && marquee_line_index < AppContext::MAX_MARQUEE_LINES) {
                String item_text = (i == list_size) ? "<- back" : context.current_playlist_files[i].path;
                if (i < list_size) {
                ...
                draw_marquee_list_item(context, marquee_line_index, 12, y_pos, item_text, i == context.selected_song_in_player);
                marquee_line_index++;
            }
        }
    }
